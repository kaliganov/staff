<?php
	...
	$firstLine = iconv( "UTF-8","cp1251", 'фамилия;имя;login;e-mail;телефон;моб. телефон;должность;департамент;город');
	...
	function get_array($connect, $ou, $base_dn)
	{
		...
		for ($i=0; $i < $info["count"]; $i++)
		{
			...
			if ($specific_ou  != ''){ 
				...
				$res[] = $famNameArr[0].';'.$famNameArr[1].';'.$info[$i]["samaccountname"][0].';'.$info[$i]["mail"][0].';'.str_replace('&nbsp;', '', $info[$i]["telephonenumber"][0]).';'.$info[$i]["mobile"][0].';'.$info[$i]["title"][0].';'.department_for_sap($info[$i]["dn"]).';'.$info[$i]["l"][0].PHP_EOL;
